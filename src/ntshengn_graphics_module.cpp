...
void NtshEngn::GraphicsModule::update(float dt) {
	...
	undefinedToColorAttachmentDependencyInfo.imageMemoryBarrierCount = 1;
	undefinedToColorAttachmentDependencyInfo.pImageMemoryBarriers = &undefinedToColorAttachmentImageMemoryBarrier;
	...
	renderingSwapchainAttachmentInfo.clearValue.color = { m_backgroundColor.x, m_backgroundColor.y, m_backgroundColor.z, m_backgroundColor.w };
	...
	if (!m_uiElements.empty()) {
		...
		sceneUIImageMemoryBarrier.image = (windowModule && windowModule->isWindowOpen(windowModule->getMainWindowID())) ? m_swapchainImages[imageIndex] : m_drawImage;
...
void NtshEngn::GraphicsModule::drawUIText(FontID fontID, const std::wstring& text, AnchorPoint anchorPoint, CoordinateType coordinateType, const Math::vec2& position, float rotation, const Math::vec2& scale, const Math::vec4& color) {
	...
	Math::vec2 min = Math::vec2(0.0f, std::min(font.topLeft.y, font.bottomRight.y));
	...
	for (const wchar_t& c : text) {
		if (c == '\n') {
			max.x = std::max(max.x, positionAdvance);
...
void NtshEngn::GraphicsModule::createDepthImage() {
	...
	NTSHENGN_VK_CHECK(vkBeginCommandBuffer(m_initializationCommandBuffer, &depthImageTransitionBeginInfo));
	...
	undefinedToDepthStencilAttachmentDependencyInfo.imageMemoryBarrierCount = 1;
	undefinedToDepthStencilAttachmentDependencyInfo.pImageMemoryBarriers = &undefinedToDepthStencilAttachmentImageMemoryBarrier;
	m_vkCmdPipelineBarrier2KHR(m_initializationCommandBuffer, &undefinedToDepthStencilAttachmentDependencyInfo);
	...
	depthImageTransitionSubmitInfo.pCommandBuffers = &m_initializationCommandBuffer;
