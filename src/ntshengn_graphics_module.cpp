...
void NtshEngn::GraphicsModule::update(float dt) {
	...
	if (m_mainCamera != NTSHENGN_ENTITY_UNKNOWN) {
		...
		cameraView = Math::quatToRotationMatrix(cameraTransform.rotation) * Math::lookAtRH(cameraTransform.position, cameraTransform.position + camera.forward, camera.up);
