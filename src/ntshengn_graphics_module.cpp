...
void NtshEngn::GraphicsModule::init() {
	if (windowModule && windowModule->isWindowOpen(windowModule->getMainWindowID())) {
		...
		NTSHENGN_MODULE_ERROR(getName() + " requires a window module and at least one open window.");
	}
	...
	for (const VkQueueFamilyProperties& queueFamilyProperty : queueFamilyProperties) {
		if ((queueFamilyProperty.queueCount > 0) && (queueFamilyProperty.queueFlags & VK_QUEUE_GRAPHICS_BIT)) {
			...
			vkGetPhysicalDeviceSurfaceSupportKHR(m_physicalDevice, m_graphicsQueueFamilyIndex, m_perWindowResources[0].surface, &presentSupport);
			if (presentSupport) {
				break;
			}
		}
