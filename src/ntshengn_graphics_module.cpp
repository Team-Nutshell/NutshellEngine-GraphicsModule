...
void NtshEngn::GraphicsModule::update(float dt) {
	...
	updateDescriptorSet(m_currentFrameInFlight, texturesDescriptorImageInfos);
	updateParticleGraphicsDescriptorSet(m_currentFrameInFlight, texturesDescriptorImageInfos);
...
void NtshEngn::GraphicsModule::updateDescriptorSet(uint32_t frameInFlight, const std::vector<VkDescriptorImageInfo>& texturesDescriptorImageInfos) {
...
void NtshEngn::GraphicsModule::createParticleResources() {
	...
	m_particleGraphicsDescriptorSetsNeedUpdate.resize(m_framesInFlight);
	for (uint32_t i = 0; i < m_framesInFlight; i++) {
...
void NtshEngn::GraphicsModule::updateParticleGraphicsDescriptorSet(uint32_t frameInFlight, const std::vector<VkDescriptorImageInfo>& texturesDescriptorImageInfos) {
	if (!m_particleGraphicsDescriptorSetsNeedUpdate[frameInFlight]) {
		return;
	}
